...
def meshfree(N, S_max, T, M, E, r, sigma, theta, c):
    ...
    V[:, M-1] = [k - E if k - E > 0 else 0 for k in S]  # Call - (max(S-E, 0)), Put - (max(E-S,0))
    for i in range(N):
        for j in range(N):
            phi[i, j] = np.exp(-np.abs(S[i]-S[j])**2/c**2)
            R[i, j] = 1/2*sigma**2*S[i]**2*((4*(S[i]-S[j])**2-2*c**2)/c**4)*\
                      phi[i, j] + r*S[i]*(-2*(S[i]-S[j])/c**2)*phi[i, j] + r*phi[i, j]
